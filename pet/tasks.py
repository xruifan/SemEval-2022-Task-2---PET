...
import re           # import re for splitting mwe       --Xuan-Rui Fan
...
"""
Custom task data processor      --Xuan-Rui Fan
"""
    ...
    def _create_examples(path, set_type, max_examples=-1, skip_first=0):
        ...
        with open(path, encoding='utf8') as f:
            ...
            for idx, row in enumerate(reader):
                ...
                label = row['Label']
                mwe = row['MWE']
                mwe_second_word = re.split('\s+|-', mwe)[-1]

                guid = "%s-%s" % (set_type, idx)

                meta = {
                    'mwe': mwe,
                    'mwe_second_word': mwe_second_word
                }

                example = InputExample(guid=guid, text_a=sentence, label=label, meta=meta)
                examples.append(example)
...
PROCESSORS = {
    ...
    "ax-g": AxGProcessor,
    "ax-b": AxBProcessor,
    "idiom-detection": MyTaskDataProcessor,         # added custom data processor       --Xuan-Rui Fan
...
DEFAULT_METRICS = ["f1-macro"]          # changed difault metrics to f1-macro       --Xuan-Rui Fan
